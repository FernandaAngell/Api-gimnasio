...
from backend.models import Progreso, Usuario
from backend.schemas import ProgresoCreate, ProgresoResponse
...
def registrar_progreso(data: ProgresoCreate, db: Session = Depends(get_db)):
    ...
    progreso = Progreso(usuario_id=data.usuario_id, peso=data.peso, fecha=data.fecha)
    db.add(progreso)
    db.commit()
    db.refresh(progreso)
    return progreso
...
def ver_progreso(
    ...
    return query.order_by(Progreso.fecha).all()

@router.get("/{usuario_id}/cambio")
def cambio_peso(usuario_id: int, db: Session = Depends(get_db)):
    registros = db.query(Progreso).filter(Progreso.usuario_id == usuario_id).order_by(Progreso.fecha).all()
    if len(registros) < 2:
        raise HTTPException(status_code=400, detail="Se necesitan al menos 2 registros")
